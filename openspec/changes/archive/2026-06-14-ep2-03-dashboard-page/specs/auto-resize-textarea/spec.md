...
### Requirement: User can input text and submit to generate video (PARTIALLY MODIFIED)

The system SHALL retain the core functionality of accepting text input and calling `project.createAndGenerate`, but the UI implementation changes from large textarea to auto-resizing input.

#### Scenario: Empty text disables submit button (UNCHANGED)

- **WHEN** the input is empty or contains only whitespace
- **THEN** the "生成" button is disabled

#### Scenario: Submission in progress disables button and input (UNCHANGED)

- **WHEN** a submission is in progress (`createMutation.isPending` is true)
- **THEN** the "生成" button shows a spinner and is disabled
- **AND** the input is disabled
- **AND** clicking the button again does not trigger a second request

#### Scenario: Submission failure shows error toast (UNCHANGED)
...
### Requirement: Generate button is positioned below the textarea (REMOVED)

This requirement is replaced by the inline positioning requirement above. The button no longer sits in a separate flex container but is integrated with the input layout.
